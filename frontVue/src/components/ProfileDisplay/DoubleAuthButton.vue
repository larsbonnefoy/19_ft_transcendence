...
let submit = (async () => {
        try {
            const data = await axios.post(`http://${import.meta.env.VITE_LOCAL_IP}:${import.meta.env.VITE_BACKEND_PORT}/twofa/verify/`, {token: localStorage.getItem('jwt_token'), code: digits2fa.value});
            if (data.data == true) {
...
async function toggle() {
    if (TwoFactor.value == false) {
        try {        
            const data = await axios.post(`http://${import.meta.env.VITE_LOCAL_IP}:${import.meta.env.VITE_BACKEND_PORT}/twofa/create/`, {token: localStorage.getItem('jwt_token')});
